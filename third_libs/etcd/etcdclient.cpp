...
ResponseError *checkForError(Document &resp) {
  if( resp.HasParseError()  ) {
	  cout<<"document parse error original content : "<<endl;
	return new ResponseError(
	resp.GetParseError(),
	"parse json error",
	to_string(resp.GetErrorOffset()),
	-1
	);
  }
...
unique_ptr<PutResponse> Session::putDirectory(string key){
	Host &host = nextHost();
	string url = base_url(host, key);
	url.append("?dir=true&prevExist=false");	
	ostringstream postData;
	//postData << "value=" << value;
	return putAndPostHelper(url, postData.str(), true);
}
